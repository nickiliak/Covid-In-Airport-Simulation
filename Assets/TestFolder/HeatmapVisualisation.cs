...
public class HeatmapVisualisation
{
    ...
    {
        Vector3Int eventPositionInParticleGrid = heatmapParticleSystem.ConvertGlobalPositionToParticleGrid(eventPosition, settings);
        Vector3Int sizeInParticles = heatmapParticleSystem.GetSizeOfParticleSystemInParticles();
        ...
        for (int x = minBound.x; x <= maxBound.x; x += 1)
        {
            for (int y = minBound.y; y <= maxBound.y; y += 1)
            {
                for (int z = minBound.z; z <= maxBound.z; z += 1)
                {
                    ...
                    {
                        UpdateColorAddValue(new Vector3Int(x, y, z), eventPositionInParticleGrid, 1);
                    }
                }
            }
        }
    ...
    private void UpdateColorAddValue(Vector3Int particlePositionInGrid, Vector3Int eventPositionInParticleGrid, int Multiplier)
    {
        ...
        if (distance < settings.maxColoringDistance)
        {
            ...
            particleColorValues[particlePositionInGrid.x, particlePositionInGrid.y, particlePositionInGrid.z] += colorAddValue * Multiplier;
        }
    }
